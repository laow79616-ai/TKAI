"""Immutable contracts for the metadata-only Hyper Intelligence Fabric."""
...
@dataclass(frozen=True)
class HyperIntelligenceProfile:
...
class CompatibilityRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if not generations.issubset({"", "v6", "v7", "v8"}):
            raise ValueError("compatibility generations must be V6, V7, or V8")
        object.__setattr__(self, "metadata", immutable_metadata(self.metadata))


Reference = IntelligenceReference
IntelligenceProfile = HyperIntelligenceProfile
