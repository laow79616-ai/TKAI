"""RBAC, isolation, secret filtering, and non-execution guards."""
# ruff: noqa: E501
...
token_trace hidden_prompt system_prompt model_weights secret_context""".split()
)


def validate_operations_metadata(metadata: Mapping[str, object]) -> None:
...
def authorize_metadata_read(
    ...
    if not {"reader", "auditor", "operations-metadata-reader"}.intersection(
        role_references
    ):
        raise PermissionError("RBAC operations metadata read denied")
