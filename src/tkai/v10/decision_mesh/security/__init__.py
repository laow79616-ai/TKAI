...
def validate_decision_metadata(metadata: Mapping[str, object]) -> None:
...
def authorize_metadata_read(
    ...
    if not {"reader", "auditor", "decision-metadata-reader"}.intersection(
        role_references
    ):
        raise PermissionError("RBAC decision metadata read denied")
