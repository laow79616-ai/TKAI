"""Local-first, bounded, advisory TKAI V10 Sovereign Planning Mesh."""
# ruff: noqa: E501
...
from tkai.v10.planning_mesh.contracts import (  # noqa: F401
    Dependency,
    DependencyType,
    Milestone,
    Objective,
...
from tkai.v10.planning_mesh.registry import PlanningMeshRegistry
from tkai.v10.planning_mesh.security import filter_secrets, validate_planning_metadata
from tkai.v10.planning_mesh.validation import MAX_RESULT_SIZE, validate_record
...
class SovereignPlanningMesh:
    """Stores planning-reference metadata; never schedules, allocates, or executes."""

    def __init__(self, *, per_registry_limit: int = 1_000) -> None:
        self.registries = PlanningMeshRegistry(per_registry_limit=per_registry_limit)
    ...
    def _safe(record: object) -> None:
        for name in ("safe_metadata", "metrics"):
            ...
            if metadata is not None:
                validate_planning_metadata(metadata)
        for item in (
            fields(record)
    ...
    def register(self, registry: str, record: object) -> object:
        ...
        self._audit.append(
            {
                "action": "planning-metadata-registered",
                "subject": registry,
    ...
    def metrics(self) -> dict[str, int]:
        ...
        values = {
            f"v10_planning_{metric}_total": len(self.registries.get(registry))
            for metric, registry in mapping.items()
        }
        values["v10_planning_validation_failures_total"] = self._validation_failures
        values["v10_planning_health_status"] = 1
        return values
    ...
    def overview(self) -> dict[str, object]:
        return {
            "mesh_id": "tkai-v10-sovereign-planning-mesh",
            "version": "10.0.0",
    ...
    def serialize(value: Any) -> Any:
        ...
        if isinstance(value, dict):
            ...
            return {str(k): SovereignPlanningMesh.serialize(v) for k, v in safe.items()}
        if isinstance(value, (tuple, list, set, frozenset)):
            return [SovereignPlanningMesh.serialize(v) for v in value]
