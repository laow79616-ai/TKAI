...
def test_repository_and_package_structure() -> None:
    ...
    required = {
        "autonomous_core", "intelligence", "knowledge", "reasoning", "decision",
        "planning", "operations", "recovery", "governance", "trust", "integrity",
        "compatibility", "contexts", "registries", "relationships", "contracts",
        "interfaces", "validation", "diagnostics", "health", "metrics", "audit",
        "security", "events", "dashboard", "api",
    }
...
def test_api_contains_no_forbidden_endpoint() -> None:
    forbidden = (
        "execute", "action", "login", "browser", "deploy", "schedule",
        "allocate", "mutate", "control", "tiktok",
    )
