...
def profile(scope: Scope = SCOPE) -> IntelligenceProfile:
    return IntelligenceProfile(
        "p1", "profile", "description", "owner", scope, "30-days"
    )
...
def decision(**changes: object) -> DecisionRecord:
    values = {
        "decision_id": "d1",
        "profile_reference": "p1",
        "context_reference": "c1",
...
def test_profiles_contexts_lifecycle_and_scope_isolation() -> None:
    ...
    context = IntelligenceContext(
        "c1", "p1", "subject", "ref://subject", (NOW, NOW), SCOPE
    )
